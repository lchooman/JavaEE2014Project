...
import com.lewischooman.models.HallDB;
...
public class HallDAO implements IHallDAO {
    ...
    public List<HallDB> getHallsByTheater(Integer theaterId) {
        Session session;
        Criteria criteria;

        session = this.sessionFactory.getCurrentSession();
        criteria = session.createCriteria(HallDB.class, "hl");
        if (theaterId != null) {
            criteria.createAlias("hl.theater", "the");
            criteria.add(Restrictions.eq("the.id", theaterId));
        }
        return (List<HallDB>) criteria.list();
    }
}
